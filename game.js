/* 한 판만 v7 — virtual currency only. Economic actions are synchronous and saved atomically. */
(()=>{'use strict';
...
if(!['betAmount','economyBar','homeNote','saveWarning','bossBanner','runTrack','handPreview','lifeStatus'].every(id=>$(id))){
 const recovery=new URL('index.html',location.href);recovery.searchParams.set('v','7');
...
function fresh(difficulty='hard',history=[]){return{version:5,mode:difficulty,money:MODES[difficulty].start,debt:0,due:0,bills:0,home:0,owned:[],counts:{},day:1,good:0,memories:0,bestStreak:0,peak:MODES[difficulty].start,lastNight:0,night:null,history,status:'alive',sound:false,milestones:[],events:[],lifeEvent:null,eventHistory:[],eventRecent:[],effects:[],nextEventDay:2,log:['새 인생. 시작금은 이번 한 번뿐입니다.']}}
...
function eventCatalog(){
 ...
 const bu=b?round(Math.max(250,b.price*.2)):0,hu=round(Math.max(180,HOMES[s.home][2]*.08)),iu=item?round(Math.max(120,item.price*.4)):0;
 ...
 if(b){
  ...
  add('buyout','사업','💼','사업을 사고 싶다는 사람',`${b.name}을 눈여겨본 매수자가 찾아왔습니다. 성사되면 사업과 향후 수입·운영비를 넘깁니다.`,[
   choice('확정 제안을 받는다',0,[outcome(100,'계약서에 서명하고 열쇠를 넘겼습니다.',round(b.price*.8),{remove:b.id})]),
   choice('더 큰 인수 금액을 요구',bu,[outcome(35,'협상이 통했습니다. 기대 이상의 매각입니다.',round(b.price*2.2),{remove:b.id}),outcome(65,'협상이 결렬됐습니다. 자문료만 지출했고 사업은 남았습니다.')]),skip('계속 직접 운영한다')]);
...
function resolveLifeEvent(index){
 ...
 if(o.remove){s.owned=s.owned.filter(id=>id!==o.remove);s.effects=s.effects.filter(f=>!(f.kind==='revenue'&&f.target===o.remove))}
...
function checkBankruptcy(){if(s.status!=='alive'||s.night||s.lifeEvent)return false;if(s.money+assets()+credit()<required()){s.status='bankrupt';log('더 이상 버틸 자산이 없어 파산했습니다.');save();return true}return false}
function header(){
...
function renderNight(){const n=s.night;if(!n)return;$('tableLabel').textContent=`${n.table} / 6`;$('chipLabel').textContent=fmt(s.money);$('potLabel').textContent=fmt(n.pot);$('riskLabel').textContent=`×${n.risk}`;$('betAmount').textContent=fmt(bet());$('deviceCount').textContent=`${n.devices.length} / ${modern(n)?3:5}`;
 $('deviceRack').innerHTML=n.devices.length?n.devices.map(id=>{const d=DEVICES.find(x=>x.id===id);return`<div class="device"><b>${d.name}</b><small>${d.text}</small></div>`}).join(''):'<span class="empty">목표를 채우면 밤 상점에서 장치를 구입할 수 있습니다</span>';
...
function sell(id){if(blockedByEvent())return;const g=good(id);if(!g?.sellable||!owns(id)||s.status!=='alive'||s.night)return;modal(`${g.name} 판매`, `<p>구입가 ${fmt(g.price)} → 중고 판매가 <b>${fmt(resale(g))}</b></p><p>${g.tab==='business'?'매각하면 이 사업의 수입과 운영비도 사라집니다.':'나중에 다시 살 때는 원래 가격을 내야 합니다.'}</p>`,[['판매한다',()=>{if(!owns(id))return;s.owned=s.owned.filter(x=>x!==id);clearAssetEffects('revenue',id);s.money+=resale(g);if(id==='car')milestone('sold-car','스포츠카 열쇠를 건넸다. 화려했던 밤은 기억에 남았다.');log(`${g.name}을 ${fmt(resale(g))}에 판매`);closeModal();afterAction()},'primary'],['취소',closeModal]])}
function confirmHome(i){if(blockedByEvent())return;if(!HOMES[i]||i===s.home||s.status!=='alive'||s.night)return;const delta=HOMES[i][2]-Math.floor(HOMES[s.home][2]*.8);if(delta>s.money||i>s.home&&restricted())return;modal(`${HOMES[i][1]} 이사`, `<p>${delta>=0?'지출':'회수'} ${fmt(Math.abs(delta))}<br>하루 주거비 ${fmt(HOMES[s.home][3])} → ${fmt(HOMES[i][3])}<br>현재 집에 붙은 사건 영향은 이사하면 끝납니다.</p>`,[['이사한다',()=>{s.money-=delta;if(i<s.home)milestone('smaller-home','집을 줄여 다시 버틸 돈을 마련했다.');clearAssetEffects('upkeep',s.home);if(i>0)milestone('first-home','내 이름으로 된 첫 집의 문을 열었다.');s.home=i;log(`${HOMES[i][1]} 이사`);closeModal();afterAction()},'primary'],['취소',closeModal]])}
function borrow(amount){if(blockedByEvent())return;if(s.status!=='alive'||s.night||credit()<amount)return;modal('대출 계약 확인',`<p>${fmt(amount)}를 빌립니다. 하루 복리 ${mode().interest*100}%.</p><p>상환일: <b>${s.debt?s.due:s.day+5}일째</b>. 만기에는 남은 원금과 이자를 모두 갚아야 합니다. 재대출로 만기는 늘어나지 않습니다.</p>`,[['빌린다',()=>{if(credit()<amount)return;if(!s.debt)s.due=s.day+5;s.debt+=amount;s.money+=amount;log(`${fmt(amount)} 대출 · ${s.due}일 만기`);closeModal();afterAction()},'primary'],['취소',closeModal]])}
